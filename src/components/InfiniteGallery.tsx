...
function ImagePlane({
  ...
  useEffect(() => {
    const meshMaterial = meshRef.current?.material as THREE.ShaderMaterial | undefined;
    if (meshMaterial?.uniforms) {
      meshMaterial.uniforms.map.value = texture;
    }
  }, [texture]);
  ...
  useEffect(() => {
    const meshMaterial = meshRef.current?.material as THREE.ShaderMaterial | undefined;
    if (meshMaterial?.uniforms) {
      meshMaterial.uniforms.isHovered.value = isHovered ? 1.0 : 0.0;
    }
  }, [isHovered]);
...
function GalleryScene({
  ...
  const depthRange = zSpacing * Math.max(visibleCount, 1);
...
function FallbackGallery({ images }: { images: ImageItem[] }) {
  ...
  return (
    <div className="flex h-full flex-col items-center justify-center bg-background p-4">
      <p className="mb-4 text-foreground/60">WebGL not supported. Showing image list:</p>
...
function InfoOverlay({
  ...
  return (
    <div className="info-overlay fixed inset-0 z-50 flex items-center justify-center p-4 pointer-events-none">
      <div
        className="relative w-full max-w-md rounded-lg border p-6 backdrop-blur-md"
        style={{
      ...
      >
        <div
          className="mb-3 inline-block rounded-full px-3 py-1 text-xs font-mono tracking-wider"
          style={{
        ...
        <h2 className="mb-3 font-serif text-2xl leading-tight md:text-3xl" style={{ color: "#F1D088" }}>
          {data.title}
        </h2>

        <p className="text-sm leading-relaxed md:text-base" style={{ color: "rgba(255, 255, 255, 0.8)" }}>
          {data.description}
        </p>

        <div className="absolute top-2 left-2 h-4 w-4 border-l border-t" style={{ borderColor: "rgba(241, 208, 136, 0.5)" }} />
        <div className="absolute top-2 right-2 h-4 w-4 border-r border-t" style={{ borderColor: "rgba(241, 208, 136, 0.5)" }} />
        <div className="absolute bottom-2 left-2 h-4 w-4 border-l border-b" style={{ borderColor: "rgba(193, 250, 248, 0.5)" }} />
        <div className="absolute right-2 bottom-2 h-4 w-4 border-r border-b" style={{ borderColor: "rgba(193, 250, 248, 0.5)" }} />
      </div>
    </div>
  );
}
...
export default function InfiniteGallery({
  ...
  /** null = 尚未检测（SSR 与客户端首帧一致）；避免用 document 在 useState 初值里判断导致服务端 false、浏览器 true */
  ...
  useEffect(() => {
    const handleGlobalPointerUp = () => setIsInfoVisible(false);
    const handleGlobalTouchEnd = () => setIsInfoVisible(false);
